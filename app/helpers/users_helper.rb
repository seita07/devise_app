module UsersHelper
  ...
  def get_eachsubject_weekly_totaltime
    ...
    for subject in SUBJECT_LIST do
      ...
      for i in 0..6 do
        ...
        today_studytime_posts = Timepost.where('datetime': search_date.in_time_zone.all_day).where(subjects: subject).where(user_id:@user.id)
  ...
  def dayly_total_studytime
    ...
    for subject in SUBJECT_LIST do
      today_studytime_posts = Timepost.where('datetime': Date.today.in_time_zone.all_day).where(subjects: subject).where(user_id:@user.id)
  ...
  def weekly_total_studytime
    ...
    from = today - 6.day 
    studytime_posts = Timepost.where('datetime': from...today).where(user_id:@user.is)
  ...
  def month_total_studytime
    ...
    today = Time.current.at_end_of_day
    from = today - 30.day 
    studytime_posts = Timepost.where('datetime': from...today).where(user_id:@user.id)
    for post in studytime_posts do
